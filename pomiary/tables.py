    ...
    while(line_id!=len(lines)):
        ...
        #print(line_id, len(lines))
            ...
            while not lines[line_id][0].isalpha():
                ...
                average_t+= float(line[0])
                average_c+= float(str(line[1]).rstrip())
                average_e+= int(100*(((int)(str(line[1]).rstrip())-optimal)/optimal))
                i+=1
                line_id+=1
                if line_id == len(lines):
                    break
            average_t=(int(average_t/i))
            average_c=(int(average_c/i))
            average_e=(int(average_e/i))
            ...
            cell_text.append( ["{:.2e}".format(int(average_t)), int(average_c), str(int(average_e))+"%"])
            ...
            ax.table(cellText=[col_labels] + cell_text, loc='center', rowLoc='center', cellLoc='center',
                    colLabels=None, cellColours=[['#f0f0f0', '#f0f0f0', '#f0f0f0']]*(i+1)+[['#f79e31','#f79e31','#f79e31']], colWidths=[0.2, 0.3, 0.1])
